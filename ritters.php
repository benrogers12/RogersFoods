...
<head>
<meta charset=UTF-8>
<title>Ritter's LLC DB</title>
</head>
<body>
<form method='POST' action="/ritters.php">		<!-- Sets the post return action-->
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<INPUT TYPE = "Text" VALUE ="" NAME = "search">

<input type="submit" name="submit" value="Search!" />		<!--Creates the Search button-->
<br><strong>Search by entering the name or category in the search box</strong>
<br><br>
</form>

<?php
$link = mysqli_connect(localhost, "admin29Krb6S", "vCeKcj5hjSAt","ritters");
if(mysqli_connect_errno()) {
 ...
 if(isset($_POST['submit'])){           //Checks for the button to be pressed
        ...
        $data=$_POST['search'];
	//$query = "SELECT * FROM wholesale WHERE item LIKE '%$data%' OR cat LIKE '%$data%';";
	$query = "SELECT * FROM CSVImport WHERE item LIKE '%$data%' OR cat LIKE '%$data%';";
        ...
        if($rows==0){
                echo "There were no plants found.<br><br>";
        }
        else{
		if($rows==1){
                	echo "There was 1 plant found.<br><br>";
        	}
        	else{
                	echo "There were $rows plants found <br><br>";
        	}
		//printf("Category Item Price 2wk Ready Quanity In Stock");echo "<br><br>";
		echo '<table border=1><tr><th>Category</th><th>Item</th><th>Price</th><th>2 Weeks</th><th>Ready</th><th>Quantity In Stock</th>';
		while ($row = $result->fetch_array(MYSQLI_ASSOC)){
			echo '<tr><td>';
        		printf($row['cat']);echo'</td><td>';
			printf($row['item']);echo'</td><td>';
			printf('$'.$row['price']);echo'</td><td>';
			if($row['twowk'] == 1){printf("YES");}echo'</td><td>';
			if($row['ready'] == 1){printf("YES");}echo'</td><td>';
			printf($row['quanity']);
			echo '</td></tr>';
       		}
		echo '</table>';
...
$query = "SELECT * FROM CSVImport";
        $result = mysqli_query($link,$query) or die('Query failed: ');  //Checks for errors with the query 
        $rows = mysqli_num_rows($result);                       //Counts the rows in the query
        printf("Listed below is all of the inventory we currently have.");echo'<br><br><br>';
                //printf("Category Item Price 2wk Ready Quanity In Stock");echo "<br><br>";
                echo '<table border=4><tr><th>Category</th><th>Item</th><th>Price</th><th>2 Weeks</th><th>Ready</th><th>Quantity In Stock</th>';
                while ($row = $result->fetch_array(MYSQLI_ASSOC)){
                        echo '<tr><td>';                        printf($row['cat']);echo'</td><td>';
                        printf($row['item']);echo'</td><td>';
                        printf('$'.$row['price']);echo'</td><td>';
                        if($row['twowk'] == 1){printf("YES");}echo'</td><td>';
                        if($row['ready'] == 1){printf("YES");}echo'</td><td>';
